...
interface ISoulBoundMedalQueryable is ISoulBoundMedal {
     ...
     */
    function listContributors(uint256 offset, uint256 limit)
        external
        view
        returns (string memory);
}
...
contract SoulBoundMedal is ERC721, Ownable, ISoulBoundMedalQueryable {
    ...
    // contributor address => ( medal,can claim status:0 can not claim,1 has claimed,2 can claim)
    ...
    function getDao2daoBridge() public view returns (address) {
     ...
     * @param medal  the medal
     */
    function award(address contributor, uint8 medal) public override onlyOwner {
        //_awardMap
        require(_awardMap[contributor][medal] == 0, "already awarded");
        require(medal < _medals.length, "unknown medal");
        _awardMap[contributor][medal] = 2;
        // push to contributors
        if (_contributorsMap[contributor] == 0) {
            _contributorsMap[contributor] = 1;
    ...
    function awarded(address contributor, uint8 medal)
        ...
        returns (bool)
    {
        return _awardMap[contributor][medal] > 0;
    }
     ...
     * Emits a {Transfer} event.
     */
    function cliam(uint8 medal) public override {
        require(medal < _medals.length, "medal not found");
        address contributor = msg.sender;
        uint8 canClaim = _awardMap[contributor][medal];
        if (canClaim == 2) {
            _cliam(contributor, medal);
        } else if (canClaim == 1) {
            revert("already claimed");
        } else {
            // cross contract call
            if (_dao2daoBridge == address(0)) {
                revert("can not claim");
            } else {
                IDAO2DAOMedal DAO2DAOMedal = IDAO2DAOMedal(_dao2daoBridge);
                bool pass = DAO2DAOMedal.medalCrossVerify(
                    address(this),
                    contributor,
                    medal
                );
    ...
    function _cliam(address contributor, uint8 medal) private {
        _awardMap[contributor][medal] = 1;
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _medalMap[tokenId] = medal;
