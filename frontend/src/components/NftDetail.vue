// replace nft
<template>
  <el-drawer custom-class="nftdetaildrawer" v-model="nftdetaildrawer" :with-header="false" :close-on-click-modal="false"
    destroy-on-close direction="btt">
    <div class="cbox">
      <div class="cdleft">
        <el-image class="nftimg" :src="Tools.imgURL(nftinfo.uri)" fit="cover" />
      ...
      <div class="cdright">
        <div class="topbtlist">
          ...
          <span :class="{ nftbtcur: listfiltertype == 'pending' }" @click="filterlist('pending')"
            class="nftfilterbt nftbte">
            Pending({{ req.pending }})
          </span>
        </div>
        <div class="listhead">
        ...
        <div class="listcontent">
          <div class="reqline" v-for="item in reqlist">
            ...
            <!-- 鼠标悬停时显示用户信息 未实现  -->
            <a style="display:inline-block" :href="Tools.blockchainScanUrl_address(item.AddressRaw)" target="_blank"
              class="span3">{{ item.Address }}</a>
        ...
        <div class="listpage">
          <span v-for="item in pageset.pages" :class="{ curpage: item.page == pageset.curpage }"
